...
class page4TableViewController: UITableViewController {
    ...
    struct PropertyKeys {
            static let page4Cell = "page4Cell"
        }
    ...
    func fetchDrinkData() {
        ...
        let url = URL(string: "https://api.airtable.com/v0/appOQ0YX5pCtJ5XhP/Page4")!
    ...
    @IBSegueAction func showDetail(_ coder: NSCoder) -> DrinkOrderViewController? {
        ...
        return DrinkOrderViewController(coder: coder, menuinfo: drinkData!, indexPath: row)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: PropertyKeys.page4Cell, for: indexPath) as! page4TableViewCell

        let drink = drinkData?.records[indexPath.row]
        
        cell.drinkName4Label.text = drink?.fields.name
        cell.describe4Label.text = drink?.fields.describe ?? " "
        cell.medium4Label.text = "\((drink?.fields.mediumPrice)!)"
        cell.large4Label.text = "\((drink?.fields.largePrice)!)"
